...
def sigmoid(activation):
    ...
    # normalize data using standard deviation since it is otherwise too big, and would cause overflow
...
def weight_init(num_input, num_output, num_hidden):
...
def perceptron(train_input, train_output):
    ...
    # start with randomly chosen weight vector w_0
    w1, w2 = weight_init(num_input, num_output, num_hidden)
    ...
    for _ in range(n_epoch):
        for i in range(len(train_input)):
            ...
            sum2 = activation_function(w2, sum1)
            # output of output layer
            predict_out = sigmoid(sum2)


            # matrix different dimension need to reshape
            temp1 = (learning_rate * train_input[i])
            temp2 = (learning_rate * sum1)
            delta1 = []
            delta2 = []
            for row in temp1:
                insert1 = [row] * num_hidden
                delta1.append(insert1)

            for row in temp2:
                insert2 = [row] * num_output
                delta2.append(insert2)
...
def neural_network(train_input, train_output, test_input):
    predictions = []
    trained_w1, trained_w2 = perceptron(train_input, train_output)

    for row in test_input:
        hidden_out = activation_function(trained_w1, row)
        prediction = activation_function(trained_w2, hidden_out)
