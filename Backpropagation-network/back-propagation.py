...
def eval_network(actual_output, predicted_values):
    ...
    condusionMatrix = [[0] * num_class for i in range(7)]
    
    correct = 0
    for j in range(len(actual_output)):
        answer = int(actual_output[j])
        predict = int(predicted_values[j])
        condusionMatrix[answer - 1][predict - 1] += 1
        f.write('Actual Class:' + str(answer) + '| Predicted Class:' + str(predict))
        if answer == predict:
            correct += 1
            f.write('CORRECT!')
...
print('ACT\\OUT\t|   1\t|   2\t|   3\t|   4\t|   5\t|   6\t|')
for i in range(6):
    print(str('   ') + str(i + 1), end='\t|')
    for j in range(6):
        num = condusionMatrix[i][j]
        print(str('  ') + str(' ') * (num < 10) + str(num), end='\t|')
        print()
    
    print('Overall accuracy:' + str(round(correct / float(len(actual_output)), 3) * 100.0) + '%')
    print('Average Percision:', str(round(sum(precisions) / (num_class - 1) * 100, 2)) + '%')
    print('Average Recall:', str(round(sum(recalls) / (num_class - 1) * 100, 2)) + '%')
